...
> 定义：人们常常关心满足某种条件的那些样本点所组成的集合——即**随机事件**，简称**事件**。  
> 
注意：事件是一个集合。
...
举例（接着上面掷骰子的例子）：  
现在，我们想研究偶数的概率，记事件A={2, 4, 6}，求 $P(A)$。  
...
## 6.独立性
在前面的章节中，我们介绍了一些事件间的关系。现在再介绍一种，即事件间**相互独立**，简称**独立**。  

通俗来讲，**独立**就是：你发生你的，我发生我的，互不影响，反之亦然。即，事件A发生与否，不会影响到事件B发生的概率，反之亦然。这种场景在生活中很常见。

举例：抛甲、乙两个硬币观察结果，设事件A={甲为正}，事件B={乙为正}。  

常识告诉我们甲的结果不会影响乙的结果；乙的结果也不会影响甲的结果。我们来证明一下前者，下图反映了样本空间以及各事件包含基本事件的情况（左甲右乙）：

<p align="center">
<img src="imgs/independent.drawio.svg" width=400 />
</p>

分析：  
1. 不管A，直接算B发生的概率， $P(B)= (淡红色+淡黄色) / 1 = 0.5$ 。  
2. 如果A发生了，那么在A发生的条件下B发生的概率 $P(B|A) = 淡红色/(淡蓝色+淡红色) = 0.25 / 0.5 = 0.5$ 。  
3. 如果A没发生，那么在A没发生的条件下B发生的概率 $P(B|\overline{A}) = 淡绿色/(淡绿色+淡黄色) = 0.25 / 0.5 = 0.5$ 。  

结论：  
甲的结果不会影响乙的结果，即 $P(B|A)=P(B)$， $P(B|\overline{A})=P(B)$ ；同理可证乙的结果不会影响甲的结果，即 $P(A|B)=P(A)$， $P(A|\overline{B})=P(A)$ 。最终，推出事件A和事件B相互独立。

下面，我们给出严谨的数学定义：
> 定义：设A、B是两个事件，如果满足等式：
> $$P(AB)=P(A)P(B)$$
> 称事件A，B**相互独立**，简称A，B**独立**。
>
> 解读：  
...
> 推广到n个事件（n≥2）：  
> 
> 设有n个事件，如果对于其中任意2个，任意3个，...，任意n个事件的事件积的概率，都等于各事件概率之积，则称这n个事件相互独立。

我们可以将事件间的独立关系也加入到前文的关系图中：
<p align="center">
<img src="imgs/event_calculate7.drawio.svg" width=800 />
</p>

从中还可以得出一些结论（在 $P(A)>0$， $P(B)>0$的前提下）：
> - 互斥 => 不独立
> 
> - 独立 => 相容（不互斥）

## 小结
随机试验的全部可能结果组成的集合S称为**样本空间**。
样本空间S的子集称为**事件**。
当且仅当这一子集中的一个样本点出现时，称这一**事件发生**。
**事件是一个集合**，因而事件间的关系与事件的运算自然按照集合论中集合之间的关系和集合的运算来处理。

在一次试验中，一个事件(除必然事件与不可能事件外)可能发生也可能不发生，其发生的可能性的大小是客观存在的，事件发生的**频率**以及它的稳定性，表明能用一个数来表征事件在一次试验中发生的可能性的大小，我们从频率的稳定性及频率的性质得到启发和抽象，给出了**概率**的定义，我们定义了一个集合(事件)的函数 P(·)，它满足三条基本性质：1°非负性，2°规范性，3°可列可加性。这一函数的函数值P(A)就定义为事件A的概率。  

在**古典概型**的情况，对于每个事件A给出了概率P(A)=k/n。  

**条件概率公式**：
$$P(B|A)=\frac{P(AB)}{P(A)},P(A)>0$$
条件概率公式满足概率的定义，因而条件概率也是一种概率。

**乘法率公式**：
$$P(AB)=P(B|A)P(A),P(A)>0$$

概率论与数理统计中的很多内容都是在独立的前提下讨论的。对于**事件的独立性**，我们往往不是根据定义来验证而是根据实际意义来加以判断的。根据实际背景判断事件的独立性，往往并不困难.